...
int main() {
   cout << "The server B is up";



   // Read data from file
   vector<vector<vector<unsigned int>>> all_areas_table = read_all_users("data2.txt");
   vector<string> all_areas_name = read_all_areas("data2.txt");

   //cout << "Total number of areas in this file is: " << all_areas_table.size() << endl;
   ostringstream oss;
   ...
   getaddrinfo(NULL, "31319", &hints, &server_addrinfo);
   ...
   cout << " and running using UDP on port <31319>" << endl;
   ...
   while(1) {
      ...
      iss.str("");
      iss.clear();
      cout << "The server B has received request for finding possible friends of User " << user_id << " in " << area_name << endl;

      recommended_user = recommend(all_areas_table, all_areas_name, area_name, user_id, "B");
      oss << "B " << recommended_user;
